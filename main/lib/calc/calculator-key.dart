...
abstract class Keys {
  ...
  // static KeySymbol sqrt = const KeySymbol('√');
...
class CalculatorKey extends StatelessWidget {
  const CalculatorKey({super.key, required this.symbol});
  ...
  static dynamic _fire(CalculatorKey key) =>
      kc.KeyController.fire(kc.KeyEvent(key));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        width: (symbol == Keys.zero)
            ? (size * 2)
            : size, // Para que el cero tome 2 espacios en el teclado
        // width: size,
